...
public class PreAuthAction implements Action<PaymentState, PaymentEvent> {
    ...
    public void execute(StateContext<PaymentState, PaymentEvent> stateContext) {
        log.debug("Inside PreAuthAction::execute");
        Message<PaymentEvent> message = null;
        if (new Random().nextInt(10) < 8) {
            log.debug("Approved");
            message = MessageBuilder.withPayload(PaymentEvent.PRE_AUTH_APPROVED)
                    .setHeader(PaymentServiceImpl.PAYMENT_ID_HEADER, stateContext.getMessageHeader(PaymentServiceImpl.PAYMENT_ID_HEADER))
                    .build();
        } else {
            message = MessageBuilder.withPayload(PaymentEvent.PRE_AUTH_DECLINED)
                    .setHeader(PaymentServiceImpl.PAYMENT_ID_HEADER, stateContext.getMessageHeader(PaymentServiceImpl.PAYMENT_ID_HEADER))
                    .build();
        }
